<div class="sale">
    <a href="#"><img src="images/images-main/sale.png" alt="Sale"/></a>
</div>
<div class="products">
    <div class="vitamins">
        <a href="#"><img src="images/images-main/vitamins.jpeg" alt="Vitamins"/></a>
        <a href="#"><h2>Вітаміни</h2></a>
        <ul>
            <li><a href="vitaminA.php">Вітамін A</a></li>
            <li><a href="vitaminE.php">Вітамін Є</a></li>
            <li><a href="vitaminD.php">Вітамін D</a></li>
        </ul>
    </div>
    <div class="minerals">
        <a href="#"><img src="images/images-main/minerals.jpeg" alt="Minerals"/></a>
        <a href="#"><h2>Mінерали</h2></a>
        <ul>
            <li><a href="#">Магній</a></li>
            <li><a href="#">Селен</a></li>
            <li><a href="#">Цинк</a></li>
        </ul>
    </div>
    <div class="other">
        <a href="#"><img src="images/images-main/other.png" alt="Other"/></a>
        <a href="#"><h2>Інші добавки</h2></a>
        <ul>
            <li><a href="#">Ашваганда</a></li>
            <li><a href="#">Колаген</a></li>
            <li><a href="#">Риб'ячий жир</a></li>
        </ul>
    </div>
</div>
<div class="existence">
    <a href="existence"><h4>Наявність товару на складі</h4></a>
</div>

